...
int constexpr NUM_S_VALS_PER_THREAD_X = 2;
int constexpr NUM_PV_VALS_PER_THREAD_X = 2;
...
namespace causal_multihead_self_attention_version_10 {
...
PYBIND11_MODULE(causal_multihead_self_attention_version_10, m) {}
...
__global__ void causal_multihead_self_attention_kernel(float const* const Q_HBM,  // size Nxd_model
    ...
    for (int T_c_index = 0; T_c_index < T_c; T_c_index++) {
        ...
        for (int B_r_index = threadIdx.y; B_r_index < CEIL_DIV(B_r_bounds_checked_for_last_row, blockDim.y) * blockDim.y; B_r_index += blockDim.y) {
            ...
            int const left_S_val_column = NUM_S_VALS_PER_THREAD_X * threadIdx.x;
            ...
            float S_val_registers[NUM_S_VALS_PER_THREAD_X];
            #pragma unroll
            for (int S_val_index = 0; S_val_index < NUM_S_VALS_PER_THREAD_X; S_val_index++) {
                S_val_registers[S_val_index] = 0.0f;
            }
            ...
            if (row_in_bounds && left_S_val_unmasked) {
                ...
                for (int d_index = 0; d_index < d_head / 4; d_index++) {
                    ...
                    #pragma unroll
                    for (int S_val_index = 0; S_val_index < NUM_S_VALS_PER_THREAD_X; S_val_index++) {
                        int const S_val_column = left_S_val_column + S_val_index;
                        float4 const K_val_float4 = K_float4[S_val_column * (K_row_length / 4) + d_index];
                        S_val_registers[S_val_index] += Q_val_float4.w * K_val_float4.w;
                        S_val_registers[S_val_index] += Q_val_float4.x * K_val_float4.x;
                        S_val_registers[S_val_index] += Q_val_float4.y * K_val_float4.y;
                        S_val_registers[S_val_index] += Q_val_float4.z * K_val_float4.z;
                    }
                }
                ...
                for (int S_val_index = 0; S_val_index < NUM_S_VALS_PER_THREAD_X; S_val_index++) {
                    ...
                    if (S_val_unmasked) {
                        S_val_registers[S_val_index] = S_val_registers[S_val_index] / temperature;
                        S[B_r_index * B_c + S_val_column] = S_val_registers[S_val_index];

                        localSum = onlineSoftmaxSum(localMax, localSum, S_val_registers[S_val_index], 1.0f);
                        localMax = max(localMax, S_val_registers[S_val_index]);
                    }
                }
            }
            ...
            if (row_in_bounds && start_column_in_row_unmasked) {
                // Compute P and O
                for (int d_index = threadIdx.x * NUM_PV_VALS_PER_THREAD_X; d_index < d_head; d_index += blockDim.x * NUM_PV_VALS_PER_THREAD_X) {
                    float PV_val_registers[NUM_PV_VALS_PER_THREAD_X];
                    // Set PV vals to 0.
                    #pragma unroll
                    for (int PV_val_index = 0; PV_val_index < NUM_PV_VALS_PER_THREAD_X; PV_val_index++) {
                        PV_val_registers[PV_val_index] = 0.0f;
                    }

                    // Compute PV vals in "strides" of 4 elements at a time.
                    int V_B_c_index = 0;
                    for (; V_B_c_index < (column_upper_bound / 4) * 4; V_B_c_index += 4) {
                        ...
                        #pragma unroll
                        for (int PV_val_index = 0; PV_val_index < NUM_PV_VALS_PER_THREAD_X; PV_val_index++) {
                            PV_val_registers[PV_val_index] += S_val_float4.x * V[(V_B_c_index + 0) * d_head + d_index + PV_val_index];
                            PV_val_registers[PV_val_index] += S_val_float4.y * V[(V_B_c_index + 1) * d_head + d_index + PV_val_index];
                            PV_val_registers[PV_val_index] += S_val_float4.z * V[(V_B_c_index + 2) * d_head + d_index + PV_val_index];
                            PV_val_registers[PV_val_index] += S_val_float4.w * V[(V_B_c_index + 3) * d_head + d_index + PV_val_index];
                        }
                    }

                    // Compute PV vals in a "stride" of 1 element at a time.
                    for (; V_B_c_index < column_upper_bound; V_B_c_index += 1) {
                        ...
                        #pragma unroll
                        for (int PV_val_index = 0; PV_val_index < NUM_PV_VALS_PER_THREAD_X; PV_val_index++) {
                            PV_val_registers[PV_val_index] += S_val * V[V_B_c_index * d_head + d_index + PV_val_index];
                        }
                    }
                    ...
                    #pragma unroll
                    for (int PV_val_index = 0; PV_val_index < NUM_PV_VALS_PER_THREAD_X; PV_val_index++) {
                        int const OIndexForThread = B_r_index * O_row_length + d_index + PV_val_index;
                        O[OIndexForThread] = (O[OIndexForThread] * expf(S_row_old_global_max - S_row_new_global_max) * S_row_old_global_sum + PV_val_registers[PV_val_index]) / S_row_new_global_sum;
                    }
                }
            }
...
TORCH_LIBRARY(causal_multihead_self_attention_version_10, m) {
   // Note that "float" in the schema corresponds to the C++ double type
   // and the Python float type.
   m.def("causal_multihead_self_attention_torch(Tensor Q, Tensor K, Tensor V, int num_heads) -> Tensor");
 }

TORCH_LIBRARY_IMPL(causal_multihead_self_attention_version_10, CUDA, m) {
