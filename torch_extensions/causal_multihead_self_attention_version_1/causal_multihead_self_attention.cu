...
namespace causal_multihead_self_attention_version_1 {
...
PYBIND11_MODULE(causal_multihead_self_attention_version_1, m) {}
...
TORCH_LIBRARY(causal_multihead_self_attention_version_1, m) {
   // Note that "float" in the schema corresponds to the C++ double type
   // and the Python float type.
   m.def("causal_multihead_self_attention_torch(Tensor Q, Tensor K, Tensor V, int num_heads) -> Tensor");
 }

TORCH_LIBRARY_IMPL(causal_multihead_self_attention_version_1, CUDA, m) {
