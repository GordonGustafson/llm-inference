...
namespace causal_multihead_self_attention_version_12 {
...
PYBIND11_MODULE(causal_multihead_self_attention_version_12, m) {}
...
// To enable the use of float4 loads, we assume that the stride for dim 1 is 1,
// so we omit the stride_dim_1 field here.
template <typename T>
struct Tensor2D {
    T * const __restrict__ data_ptr;
    int64_t const stride_dim_0;
};

template <int d_head, int d_model, int B_c, int B_r>
__global__ void causal_multihead_self_attention_kernel(Tensor2D<float const> const Q_HBM_tensor_2D,  // size Nxd_model
                                                       Tensor2D<float const> const K_HBM_tensor_2D,  // size Nxd_model
                                                       Tensor2D<float const> const V_HBM_tensor_2D,  // size Nxd_model
                                                       Tensor2D<float> const O_HBM_tensor_2D,  // size Nxd_model
                                                       int const N) {
    ...
    for (int T_c_index = 0; T_c_index < T_c; T_c_index++) {
        ...
        for (int d_index = threadIdx.x; d_index < d_head / 4; d_index += blockDim.x) {
            for (int B_c_index = threadIdx.y; B_c_index < B_c_bounds_checked_for_last_column; B_c_index += blockDim.y) {
                ...
                K_float4[B_c_index * (K_row_length / 4) + d_index] = K_HBM_float4[row_index * (((int)K_HBM_tensor_2D.stride_dim_0) / 4) + (d_min_for_head / 4) + d_index];
                V_float4[B_c_index * (d_head / 4) + d_index] = V_HBM_float4[row_index * (((int)V_HBM_tensor_2D.stride_dim_0) / 4) + (d_min_for_head / 4) + d_index];
            }
        }
        ...
        for (int reg_tile_top_row_shm = threadIdx.y * NUM_ROWS_PER_THREAD;
            ...
            if (reg_tile_top_row_shm_in_bounds && reg_tile_bottom_left_unmasked) {
                ...
                for (int S_reg_row = 0; S_reg_row < NUM_ROWS_PER_THREAD; S_reg_row++) {
                    ...
                    int const reg_tile_right_column_shm = reg_tile_left_column_shm + NUM_COLS_PER_THREAD - 1;
                    bool const whole_reg_tile_row_in_bounds_and_unmasked = reg_tile_right_column_shm < shm_row_column_upper_bound_shm;

                    // If the whole reg tile row is in bounds, write the result as float4 to avoid excessive shared wavefronts.
                    if (whole_reg_tile_row_in_bounds_and_unmasked) {
                        S_registers[S_reg_row][0] /= temperature;
                        S_registers[S_reg_row][1] /= temperature;
                        S_registers[S_reg_row][2] /= temperature;
                        S_registers[S_reg_row][3] /= temperature;
                        float4 const S_reg_float4 = make_float4(S_registers[S_reg_row][0],
                                                                S_registers[S_reg_row][1],
                                                                S_registers[S_reg_row][2],
                                                                S_registers[S_reg_row][3]);
                        S_float4[(reg_tile_top_row_shm + S_reg_row) * (B_c / 4) + (reg_tile_left_column_shm / 4)] = S_reg_float4;
...
void causal_multihead_self_attention(Tensor2D<float const> const Q,  // size Nxd_model
                                     Tensor2D<float const> const K,  // size Nxd_model
                                     Tensor2D<float const> const V,  // size Nxd_model
                                     Tensor2D<float> const output,   // size Nxd_model
                                     int const N,
                                     int const d_model,
                                     int const num_heads) {
...
torch::Tensor causal_multihead_self_attention_torch(torch::Tensor Q,
    ...
    TORCH_CHECK(Q.stride(1) == 1, "Q's dim 1 stride must be 1 for float4 loads to work");
    TORCH_CHECK(K.stride(1) == 1, "K's dim 1 stride must be 1 for float4 loads to work");
    TORCH_CHECK(V.stride(1) == 1, "V's dim 1 stride must be 1 for float4 loads to work");

    TORCH_CHECK(Q.stride(0) % 4 == 0, "Q's dim 0 stride must be a multiple of 4 for float4 loads to work");
    TORCH_CHECK(K.stride(0) % 4 == 0, "K's dim 0 stride must be a multiple of 4 for float4 loads to work");
    TORCH_CHECK(V.stride(0) % 4 == 0, "V's dim 0 stride must be a multiple of 4 for float4 loads to work");
    ...
    Tensor2D<float const> Q_tensor_2D {Q.data_ptr<float>(), Q.stride(0)};
    Tensor2D<float const> K_tensor_2D {K.data_ptr<float>(), K.stride(0)};
    Tensor2D<float const> V_tensor_2D {V.data_ptr<float>(), V.stride(0)};
    Tensor2D<float> output_tensor_2D {output.data_ptr<float>(), output.stride(0)};
...
TORCH_LIBRARY(causal_multihead_self_attention_version_12, m) {
   // Note that "float" in the schema corresponds to the C++ double type
   // and the Python float type.
   m.def("causal_multihead_self_attention_torch(Tensor Q, Tensor K, Tensor V, int num_heads) -> Tensor");
 }

TORCH_LIBRARY_IMPL(causal_multihead_self_attention_version_12, CUDA, m) {
